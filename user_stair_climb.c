...
if (b1 == XM_BTN_CLICK) {
static bool bias_done_temp = false;
s_cal_state = bias_done_temp ? CAL_EMG_MVIC_RUN : CAL_EMG_BIAS_RUN;
bias_done_temp = !bias_done_temp;
...
if (b2 == XM_BTN_CLICK) {
static bool fsr_off_done_temp = false;
s_cal_state = fsr_off_done_temp ? CAL_FSR_ON_RUN : CAL_FSR_OFF_RUN;
fsr_off_done_temp = !fsr_off_done_temp;
