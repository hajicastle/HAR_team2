"""
XM10 bilateral 4-FSR gait event monitor — Stair-Ascent Final Project variant.

This is a Team 2 fork of `cdc_bilateral_gait_event_monitor.py` adapted to the
**16-channel Final_Stair_Assist** stream produced by
`XM_Apps/User_Algorithm/user_app.c`. The vendor monitor expects four floats
(PF3..PF6 as LH, LT, RH, RT FSRs); our firmware streams sixteen floats with the
FSR loads at slots 5..8 in the order (RH, RT, LH, LT). Pointing the vendor
monitor at our stream silently mis-reads the first four EMG envelopes as FSRs,
which is why this fork exists.

What this script does
---------------------
- Reads the 16-channel Final_Stair_Assist stream on USB-CDC module 0xF0.
- Extracts the four FSR loads from slots 5..8 (zero-indexed: values[4..7]).
- Re-orders (RH, RT, LH, LT) -> (LH, LT, RH, RT) so the upstream gait FSM and
  STATE_TABLE work unchanged (LH=bit3, LT=bit2, RH=bit1, RT=bit0 — same bit
  layout the firmware uses in `user_app.c:_UpdateFoot`).
- Runs the same two-step calibration, contact detection (hysteresis), fuzzy
  membership visualization, and bilateral gait-event detection as the original.
- Emits the same public event set:
      L_HEEL_STRIKE, R_HEEL_STRIKE, L_TOE_OFF, R_TOE_OFF,
      L_SUPPORT_START, R_SUPPORT_START, DOUBLE_SUPPORT, STANDING

Use this BEFORE recording a B0/E1 trial to validate FSR placement, wiring, and
fuzzy thresholds in real time. Recording a CSV here is optional and produces a
*monitor* CSV (different schema from `cdc_selective_logger.py` output) — for the
report use the selective logger.

Run
---
    python cdc_bilateral_gait_event_monitor_stairs.py --port COM6
    python cdc_bilateral_gait_event_monitor_stairs.py --port /dev/ttyACM0
    python cdc_bilateral_gait_event_monitor_stairs.py --list-ports
...
# The Final_Stair_Assist stream (user_app.c StairStreamData_t, 16 floats):
#   0: EMG R env       4: FSR RH         8:  Phase R       12: Thigh R
#   1: EMG L env       5: FSR RT         9:  Phase L       13: Thigh L
#   2: EMG R act       6: FSR LH         10: Gait R        14: Tau R
#   3: EMG L act       7: FSR LT         11: Gait L        15: Tau L
#
# This monitor's contacts list / BITS tuple uses (LH, LT, RH, RT) order so the
# bilateral state machine (STATE_TABLE) and bitmask convention match the vendor
# tool 1:1. We therefore reorder our stream's (RH, RT, LH, LT) to (LH, LT, RH, RT)
# by picking values[6], values[7], values[4], values[5].
STAIRS_MIN_CHANNELS = 16
FSR_STREAM_INDICES = (6, 7, 4, 5)   # -> (LH, LT, RH, RT)
DEFAULT_LABELS = ["LH", "LT", "RH", "RT"]

DEFAULT_SAMPLE_RATE_HZ = 100.0       # firmware streams at 100 Hz (cdc_stream_period_ms = 10)
...
DEFAULT_SAMPLE_DECIMATE = 1          # 100 Hz is already moderate; no decimation default
...
def contact_membership(value: float, off_threshold: float, on_threshold: float) -> float:
    """0 below OFF, 1 above ON, linear between; hysteresis still decides the bit."""
...
def extract_stairs_fsrs(values) -> tuple[float, float, float, float]:
    """Map the 16-channel Final_Stair_Assist payload to (LH, LT, RH, RT).

    Our firmware streams FSR loads at indices 4..7 in order (RH, RT, LH, LT).
    The monitor expects (LH, LT, RH, RT) so the STATE_TABLE / bitmask convention
    matches `_UpdateFoot` in user_app.c (mask = (lh<<3)|(lt<<2)|(rh<<1)|(rt<<0)).
    """
    return tuple(float(values[i]) for i in FSR_STREAM_INDICES)
...
class Calibration:
    ...
    def to_dict(self):
        return {
            ...
            "version": 1,
            "variant": "Final_Stair_Assist",
            "fsr_stream_indices": list(FSR_STREAM_INDICES),
        }
...
def run_gui(
            ...
            try:
                while self._running:
                    ...
                    if not chunk:
                        if now_abs - last_keepalive >= 1.0:
                            ...
                            self.status.emit(
                                ...
                                f"decoded:{decoded_count} matched:{matched_count} "
                                f"errors:{error_count} short:{short_packet_count}"
                            )
                    ...
                    while self._running:
                        ...
                        if mid != module_id:
                            continue
                        if len(values) < STAIRS_MIN_CHANNELS:
                            # The stream is the wrong one — likely a stock
                            # 4-channel app is flashed. Surface it so the user
                            # notices instead of silently mis-reading channels.
                        ...
                        fsrs = extract_stairs_fsrs(values)
                        self.sample.emit(t, seq_id, fsrs, rate_hz, error_count)
            finally:
                ser.close()
    ...
    class MainWindow(QtWidgets.QMainWindow):
        def __init__(self):
            super().__init__()
            self.setWindowTitle(
                f"XM10 Bilateral Gait Event Monitor — Stair-Ascent Final Project "
                f"(module {format_module(module_id)}, FSRs from slots 5..8)"
            )
        ...
        def _build_ui(self):
            ...
            banner = QtWidgets.QLabel(
                "Stair-Ascent variant — reading FSR loads from stream slots 5..8 "
                "(reordered to LH, LT, RH, RT). For the report CSV use the selective logger."
            )
            banner.setStyleSheet(
                "background:#fff5cc; color:#553300; border:1px solid #d8b86b;"
            ...
            self.btn_zero.clicked.connect(lambda: self.start_capture("off", 100))
            controls.addWidget(self.btn_zero)

            self.btn_on = QtWidgets.QPushButton("Capture On/Loaded")
            self.btn_on.clicked.connect(lambda: self.start_capture("on", 100))
            ...
            self.plot = pg.PlotWidget(title="Normalized FSR Loads (Final_Stair_Assist slots 5..8)")
        ...
        def save_calibration(self):
            ...
            default_path = os.path.join(default_dir, "stairs_bilateral_gait_calibration.json")
        ...
        def update_info_label(self):
            ...
            lines = [
                ...
                f"stream: Final_Stair_Assist (16 floats; FSRs at slots 5..8)",
                f"reorder: stream(RH,RT,LH,LT) -> monitor(LH,LT,RH,RT)",
        ...
        def start_recording(self):
            ...
            default_dir = os.path.join(os.path.dirname(__file__), "data")
            os.makedirs(default_dir, exist_ok=True)
            default_name = f"stairs_bilateral_gait_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "XM10 bilateral 4-FSR gait event monitor — Stair-Ascent Final Project variant. "
            "Reads the 16-channel Final_Stair_Assist USB-CDC stream and uses the FSR loads "
            "at slots 5..8 (zero-indexed 4..7) reordered to (LH, LT, RH, RT)."
        )
    )
    ...
    parser.add_argument("--module", type=parse_module_id, default=DEFAULT_MODULE_ID,
                        help="USB-CDC module id (default 0xF0 — matches user_app.c)")
    parser.add_argument("--labels", default=",".join(DEFAULT_LABELS),
                        help="Display labels in (LH, LT, RH, RT) order; rename only — order is fixed")
    ...
    if len(labels) != 4:
        print("--labels must contain exactly 4 comma-separated names "
              "(positional order is LH, LT, RH, RT)", file=sys.stderr)
        sys.exit(2)
