...
"FSR_RH", "FSR_RT",
"FSR_LH", "FSR_LT",
"Thigh_R_Deg", "Thigh_L_Deg",
"Tau_R_Nm", "Tau_L_Nm",
"Stair_Latch_R", "Stair_Latch_L"
 ]),
0xFF: ("Debug", None),
}
...
if mid == 0xF0 and len(ch_names) == 12:
return [
("1. EMG Activation (0~1)", [0, 1]),
("2. Assist Torque (Nm)", [8, 9]),
("3. FSR Load Right", [2, 3]),
("4. FSR Load Left", [4, 5]),
("5. Thigh Angle (Deg)", [6, 7]),
("6. Stair Latch Flag", [10, 11])
]
...
class PhAIPacket:
slots = ('seq_id', 'module_id', 'status', 'tx_drops', 'floats', 'recv_t')

def init(self, seq_id, module_id, status, floats, recv_t):
self.seq_id = seq_id
...
def init(self, port_name, baudrate=DEFAULT_BAUD, parent=None):
super().init(parent)
...
folder = self.edit_folder.text().strip() or "."
os.makedirs(folder, exist_ok=True)
ts = datetime.now().strftime('%Y%m%d%H%M%S')
for i, pw in enumerate(self.plot_widgets):
if self.plot_curves[i]:
path = os.path.join(folder, f"plot{i}{ts}.png")
try:
...
folder = self.edit_folder.text().strip() or os.path.abspath("data")
os.makedirs(folder, exist_ok=True)
path = os.path.join(folder, f"cdc_phai{datetime.now().strftime('%Y%m%d%H%M%S')}.csv")
...
folder = self.edit_folder.text().strip() or "."
files = sorted(glob.glob(os.path.join(folder, "cdc_phai*.csv")),
...
reviewer_script = os.path.join(os.path.dirname(file), "cdc_csv_reviewer.py")
...
def init(self, csv_path=None, parent=None):
super().init(parent)
